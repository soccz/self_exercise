...
export async function POST(req: Request) {
    ...
    if (!user.telegram_chat_id || !user.telegram_remind_enabled) {
        return NextResponse.json({ ok: true, skipped: "disabled_or_no_chat" });
    }
    ...
    if (!briefing) {
        return NextResponse.json({ ok: true, skipped: "no_analysis_data" });
    }
    ...
    return NextResponse.json({ ok: true, sent: true, briefing });
}
