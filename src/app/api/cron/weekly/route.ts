...
async function sendMessage(chatId: string, text: string): Promise<boolean> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("cron weekly: telegram send failed", res.status, detail);
      return false;
    }
    const j = (await res.json().catch(() => null)) as { ok?: unknown; description?: unknown } | null;
    if (j && j.ok === false) {
      console.error("cron weekly: telegram send failed", j.description);
      return false;
    }
    return true;
  } catch (e) {
    console.error("cron weekly: telegram send failed", e);
    return false;
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const rl = rateLimit(`cron:weekly:${getClientKey(req)}`, 30, 60_000);
    if (!rl.ok) {
    ...
    if (userError || !user) {
      console.error("cron weekly: user error", userError);
    ...
    const report = await buildWeeklyTelegramReport(supabase, SINGLE_PLAYER_ID, timeZone);
    ...
    console.error("cron weekly: unhandled error", e);
